...
public interface EventoDAOQuery
{
    ...
    String GET_EVENT_BY_ID = "select hex(e.id) as id, e.creatorid, c.title, c.description, c.localization from events e where id=unhex(?)";
    String GET_EVENT_BY_CREATOR_ID = "select hex(e.id) as id, e.creatorid, e.title, e.description, e.localization from events e where creatorid=?";
