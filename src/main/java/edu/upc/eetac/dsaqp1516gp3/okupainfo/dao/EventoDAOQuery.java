...
public interface EventoDAOQuery
{
    ...
    String GET_EVENT_BY_ID = "select hex(id) as id, hex(casalid) as casalid, title, description, localization, latitude, longitude, eventdate, creation_timestamp, last_modified from events where id=unhex(?)";
    String GET_EVENTS_BY_CASAL_ID = "select hex(id) as id, hex(casalid) as casalid, title, description, localization, latitude, longitude, eventdate, creation_timestamp, last_modified from events where casalid=unhex(?)";
    String GET_EVENTS_BY_USER_ID = "select hex(e.id) as id, hex(e.casalid) as casalid, e.title, e.description, e.localization, e.eventdate, e.latitude, e.longitude, e.creation_timestamp, e.last_modified from users u, events e, users_events uv where uv.userid=unhex(?) and u.id=uv.userid and uv.eventoid=e.id order by e.eventdate";
    ...
    String GET_ALL_EVENTS = "select hex(id) as id, hex(casalid) as casalid, title, description, localization, latitude, longitude, eventdate, creation_timestamp, last_modified from events";
