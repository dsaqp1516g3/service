...
public interface EventoDAOQuery
{
    ...
    String GET_EVENTS_BY_USER_ID = "select hex(uv.id) as id, hex(e.id) as eventid, e.casalid, e.title, e.description, e.localization from users u, events e, users_events uv where uv.userid=unhex(?) and u.id=uv.userid";
