...
public interface EventoDAOQuery
{
    ...
    String CREATE_EVENT = "insert into events((id, creatorid, title, description, localization, latitude, longitude, eventdate) values (UNHEX(?), UNHEX(?), ?,  ?, ?,NULL,NULL, ?)";
    ...
    String GET_EVENT_BY_ID = "select hex(e.id) as id, e.casalid, e.title, e.description, e.localization, e.eventdate from events e where id=unhex(?)";
    String GET_EVENT_BY_CREATOR_ID = "select hex(e.id) as id, e.casalid, e.title, e.description, e.localization, e.eventdate from events e where creatorid=?";
