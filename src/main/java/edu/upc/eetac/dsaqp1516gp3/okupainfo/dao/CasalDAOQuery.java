...
public interface CasalDAOQuery
{
    ...
    String CREATE_CASAL = "insert into casals (adminid, casalid, email, name, description, localization, latitude, longitude) values (UNHEX(?),UNHEX(REPLACE(UUID(),'-','')), ?, ?, ?, ?, ?, ?)";
    String UPDATE_CASAL = "update casals set email=?, name=?, description=? where casalid=unhex(?)";
    String GET_CASAL_BY_CASALID = "select hex(c.casalid) as casalid, hex(c.adminid) as adminid, c.email, c.name, c.description, c.localization from casals c where casalid=unhex(?)";
