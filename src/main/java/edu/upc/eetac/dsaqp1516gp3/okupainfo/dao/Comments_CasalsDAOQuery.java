...
public interface Comments_CasalsDAOQuery
{
    ...
    String GET_COMMENT_BY_ID = "select hex(cc.id) as id, hex(cc.creatorid), hex(cc.casalid) as casalid, cc.content from coments_casals cc where id=unhex(?)";
    String GET_COMMENT_BY_CASALID = "select hex(cc.id) as id, hex(cc.creatorid) as creatorid, hex(cc.casalid) as casalid, cc.content from comments_casals cc where casalid=unhex(?)";
    String GET_COMMENT_BY_CREATORID = "select hex(cc.id) as id, hex(cc.creatorid) as creatorid, hex(cc.casalid) as casalid, cc.content from comments_casals cc where creatorid=unhex(?)";
